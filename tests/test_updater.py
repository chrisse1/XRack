...
spec = importlib.util.spec_from_file_location(
    "xrack_update",
    WURZEL / "scripts" / "xrack-update.py",
)
...
try:
    ...
    if not verfuegbar:
        ...
        #
        shutil.copy(
            WURZEL / ".gitignore", quelle / ".gitignore"
        )
...
# ====================================================================
# Altlasten: Was aus dem Projekt verschwindet, verschwindet auch
# vom Gerät
#
# Der Updater kopiert nur, er löscht nicht - und das ist richtig so:
# Was er nicht sicher beurteilen kann, soll er stehen lassen. Genau
# deshalb bleibt aber auch liegen, was aus dem Projekt verschwindet.
#
# Beim Umzug der Testreihe nach tests/ wären das zweiundfünfzig
# verwaiste Dateien im Hauptverzeichnis gewesen. Deshalb gibt es eine
# ausdrückliche Liste - und sie darf genau zwei Fehler nicht machen:
# zu viel wegräumen (was jemand selbst dazugelegt hat) und am
# falschen Ort suchen (die Tests in tests/ heissen ja genauso).
# ====================================================================

with tempfile.TemporaryDirectory() as tmp:

    installation = Path(tmp) / "XRack"

    (installation / "tests").mkdir(parents=True)

    #
    # Was weg soll: die alten Testdateien im Hauptverzeichnis.
    #
    (installation / "test_alt.py").write_text("# alt", encoding="utf-8")
    (installation / "test_noch_einer.py").write_text("# alt", encoding="utf-8")

    #
    # Was bleiben muss.
    #
    (installation / "tests" / "test_neu.py").write_text("# neu", encoding="utf-8")
    (installation / "main.py").write_text("# XRack", encoding="utf-8")
    (installation / "meine_notizen.txt").write_text("privat", encoding="utf-8")

    entfernt = updater.altlasten_entfernen(installation)

    assert sorted(entfernt) == ["test_alt.py", "test_noch_einer.py"], entfernt

    assert not (installation / "test_alt.py").exists(), (
        "Die alte Testdatei liegt noch im Hauptverzeichnis."
    )

    assert (installation / "tests" / "test_neu.py").exists(), (
        "Die Tests in tests/ wurden mit weggeräumt - das Muster greift "
        "eine Ebene zu tief."
    )

    assert (installation / "main.py").exists(), installation

    assert (installation / "meine_notizen.txt").exists(), (
        "Eine Datei, die jemand selbst dazugelegt hat, wurde gelöscht. "
        "Der Updater darf nur wegräumen, was ausdrücklich auf der "
        "Liste steht."
    )

    #
    # Und ein zweites Mal ist es still: Nichts mehr zu tun ist kein
    # Fehler.
    #
    assert updater.altlasten_entfernen(installation) == []

print("OK: Alte Testdateien werden weggeräumt, alles andere bleibt")


# ====================================================================
# ... und das geschieht auch WÄHREND eines Updates
#
# Die Prüfung oben ruft das Aufräumen selbst auf - sie sagt also
# nichts darüber, ob der Ablauf es überhaupt tut. Genau das war hier
# die Lücke: Die Funktion konnte tadellos arbeiten und trotzdem nie
# drankommen.
# ====================================================================
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    install = scratch / "install-altlast"
    install.mkdir()
    build_install(install)

    #
    # Eine Testdatei aus einer früheren Fassung, so wie sie auf einem
    # Gerät liegt, das vor dem Umzug installiert wurde.
    #
    (install / "test_von_frueher.py").write_text("# alt", encoding="utf-8")

    zip_path = scratch / "neu.zip"
    build_zip(zip_path)

    updater.WORK_DIR = scratch / "arbeit"
    updater.BACKUP_DIR = updater.WORK_DIR / "backup"
    updater.EXTRACT_DIR = updater.WORK_DIR / "entpackt"
    updater.STATUS_FILE = updater.WORK_DIR / "status.json"
    updater.LOG_FILE = updater.WORK_DIR / "update.log"
    ...
    assert not (install / "test_von_frueher.py").exists(), (
        "Nach einem vollständigen Update liegt die alte Testdatei noch "
        "im Hauptverzeichnis - das Aufräumen wird im Ablauf gar nicht "
        "aufgerufen."
    )

    assert (install / "main.py").exists(), (
        "Das Update hat die Installation beschädigt."
    )

print("OK: Der Ablauf räumt die Altlasten wirklich weg")
